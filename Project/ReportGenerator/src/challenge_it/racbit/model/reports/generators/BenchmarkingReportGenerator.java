...
public class BenchmarkingReportGenerator implements IReportGenerator {
	...
	private int fill(Workbook workbook, Sheet sheet, HashMap<String, CrossReference> brokers, Map<String, BenchmarkingLocation> locations, short color, int offset) {
		...
		for (BenchmarkingLocation location : locations.values()) {
			for(BenchmarkingGroup group : location.getGroups().values()){
				for(BenchmarkingDay day : group.getDays().values()){
					...
					row = sheet.getRow(FIRST_CELL.getRow() + offset);
			...
			sheet.addMergedRegion(new CellRangeAddress(
					LOCATION.getRow() + groupOffset, //first row (0-based)
					LOCATION.getRow() + (offset-groupOffset-1) + groupOffset, //last row  (0-based)
					LOCATION.getColumn(), //first column (0-based)
					LOCATION.getColumn()  //last column  (0-based)
					));
